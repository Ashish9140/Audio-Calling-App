...
export const useWebRTC = (roomId, user) => {
    ...
    useEffect(() => {
        ...
        const setMute = (mute, userId) => {
            const cliendIdx = clietsRef.current.map(client => client.id).indexOf(userId);
            const connectedClients = JSON.parse(
                JSON.stringify(clietsRef.current)
            );
            if (cliendIdx > -1) {
                connectedClients[cliendIdx].muted = mute;
            }
